...
#define WORDS64     (TOTAL_BYTES / 8)       // 256 bytes / 8 = 32 words
...
int main(void) {
    ...
    // --- Extract last 136 bits (17 bytes) ---
    uint8_t *bytes = (uint8_t *)data64;  // reinterpret as byte array (little-endian)
    printf("Last 136 bits (reg_ro_out[2047:1912]) in binary:\n");

    for (int i = TOTAL_BYTES - LAST_BYTES; i < TOTAL_BYTES; i++) {
