...
class EosVisualizer():
    ...
    def __init__(self, filepath):
        ...
        self.nEntries = int(self.h5file[list(self.h5file.keys())[0]].attrs['ns_sample'])
    ...
    def plot_event(self, event=-1, figpath=None):
        """Display a single Eos event"""
        if event==-1: # pick random event
            #events = [np.random.randint(self.nEntries)]
            events = [0]

        hits, charges = self.sorthits(events)
        self.plot(hits, charges, figpath)
        self.clear_charge()

    def plot_multiple(self, nEvents=-1, figpath=None):
        """Display multiple Eos events overlayed"""

        if nEvents<0 or nEvents>=self.nEntries:
            nEvents = self.nEntries
    ...
    def plot(self, hits, charges, figpath=None):
        ...
        for pmtid in range(self.NPMTS):
            ...
            if pmtpos[2] == -1:
                continue
            if pmtpos[2]>900:
                top_ax.text(pmtpos[0], -1*pmtpos[1], str(pmtid), color="w", size=24, ha="center")
            elif pmtpos[2]> -700 and pmtpos[2]<900:
                phi = xy_phi(pmtpos[0], pmtpos[1])
                r_pos = 4*r*phi/(2*np.pi)
                mid_ax.text(r_pos, pmtpos[2], str(pmtid), color="w", size=24, ha="center")

            elif pmtpos[2]< -700 and pmtpos[2]> -1300:
                dic_hi_ax.text(pmtpos[0], pmtpos[1], str(pmtid), color="w", size=24, ha="center")
            elif pmtpos[2]< -1300:
                dic_lo_ax.text(pmtpos[0], pmtpos[1], str(pmtid), color="w", size=24, ha="center")
        ...
        for i in range(len(rticks)):
            mid_ax.text(rticks[i], 700, labels[i], color="w", size=24, ha="center")
        ...
        #        continue
        #    if pmtpos[2]>900:
        #        top_ax.text(pmtpos[0], -1*pmtpos[1], "LCN:%d\nx:%.2f\ny:%.2f\nz:%.2f"%(pmtid,pmtpos[0],pmtpos[1],pmtpos[2]),
        #                color="w", size=12, ha="center")
        #    elif pmtpos[2]> -700 and pmtpos[2]<900:
        #        phi = xy_phi(pmtpos[0], pmtpos[1])
        #        r_pos = 4*r*phi/(2*np.pi)
        #        mid_ax.text(r_pos, pmtpos[2], "LCN:%d\nx:%.2f\ny:%.2f\nz:%.2f"%(pmtid,pmtpos[0],pmtpos[1],pmtpos[2]),
        #                color="w", size=12, ha="center")
        #    elif pmtpos[2]< -700 and pmtpos[2]> -1300:
        #        dic_hi_ax.text(pmtpos[0], pmtpos[1], "LCN:%d\nx:%.2f\ny:%.2f\nz:%.2f"%(pmtid,pmtpos[0],pmtpos[1],pmtpos[2]),
        #                color="w", size=12, ha="center")
        #    elif pmtpos[2]< -1300:
        #        dic_lo_ax.text(pmtpos[0], pmtpos[1], "LCN:%d\nx:%.2f\ny:%.2f\nz:%.2f"%(pmtid,pmtpos[0],pmtpos[1],pmtpos[2]),
        #                color="w", size=12, ha="center")
...
if __name__ == "__main__":
    ...
    parser.add_argument("f", action="store", help="pass in hdf5 file to plot")
    parser.add_argument("-s", "--single", action="store_true", help="plot event display of single event. default is to plot multiple")
    parser.add_argument("-e", "--events", type=int, default=-1, help="number of events to be overlayed, when plotting multiple events OR which event to be shown, when plotting single event")
    ...
    figpath = os.path.join(plotdir, args.f.split("/")[-1].replace(".h5", ".png"))

    if args.single:
        EosViz.plot_event(event=args.events, figpath=figpath)
    else:
        EosViz.plot_multiple(nEvents=args.events, figpath=figpath)
